...
async def orchestrate_full_analysis(file_path: str, output_path: str):
    ...
    printd(df)
    # printd(file_stream) #FIXME: HERE WE HAVE A PROBLEM. SOMETHING NOT WORKING. UNDER THIS LINE EVERYTHING MUST BE FINE. LOOK BELOW THIS LINE.
    #TODO: EVERYTHING WORK WITHOUT VENV, ONLY WITH VENV ACTIVE MY APP STOP WORKING!!!
    buffer = BytesIO()
    printd(buffer)
    printd('before with ><<>><')
    try:
    ...
    printd('before json_data = xlsx_to_json(buffer)')
